...
import { nowMs, formatUtcMs, toBeijingHourKey, toBeijingDateKey, toBeijingHour } from "../utils/time";
...
export async function getRequestStats(db: Env["DB"]): Promise<RequestStats> {
  ...
  for (const r of last24) {
    const key = toBeijingHourKey(r.timestamp);
  ...
  for (let i = 0; i < 24; i++) {
    ...
    const key = toBeijingHourKey(ts);
    const h = toBeijingHour(ts);
  ...
  for (const r of last14) {
    const key = toBeijingDateKey(r.timestamp);
